---
description: Report one or more bugs. Describe each bug and I'll create individual markdown files in docs/bugs/ with reproduction steps, expected behavior, and an implementation plan. Runs in the background so you can keep working.
---
...
2. Create a markdown file in `docs/bugs/` named `BUG-NNN-short-description.md` where NNN is the next sequential number (check existing files first with Glob)
...
4. After creating all bug files, read the existing `docs/bugs/BACKLOG.md` and update it with any new bugs added to the table, keeping existing entries intact
